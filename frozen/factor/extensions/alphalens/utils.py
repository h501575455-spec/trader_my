...
def compute_forward_returns(
    ...
    for period in sorted(periods):
        ...
        delta_days = (
            period_len.components.days - mode(days_diffs, keepdims=True).mode[0]  # mode(days_diffs, keepdims=True).mode[0]
        )
...
def makefile_if_not_exists(config: "FrozenConfig"):

    OUTPUT_PATH = config.strategy_folder
    al_filename = "alphalens_report"

    FULL_PATH = os.path.join(OUTPUT_PATH, al_filename)
