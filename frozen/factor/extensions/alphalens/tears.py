...
def save_plot(fig, name, render=None, **kwargs):
    """
    Save alphalens plots to PNG or HTML.
    ...
    name: str
        The figure name to use as filename.

    render: Union[None, str]
        - "png": picture
        - "html": web page
        - None: save nothing
    """
    ...
    if render:
        ...
        save_path = utils.makefile_if_not_exists(output_dir)
        if render == "png":
            fig.savefig(os.path.join(save_path, f"{name}.png"), format="png", dpi=300, bbox_inches='tight')
        elif render == "html":
            html_str = mpld3.fig_to_html(fig)
            with open(os.path.join(save_path, f"{name}.html"), "w") as f:
                f.write(html_str)
        else:
            raise ValueError(f"Unsupported render type: {render}")
...
def create_returns_tear_sheet(
    factor_data, long_short=True, group_neutral=False, by_group=False, render=None, **kwargs
):
    """
    ...
    plotting.plot_quantile_returns_violin(
        mean_quant_rateret_bydate, ylim_percentiles=(1, 99), ax=ax
    )
    ...
    if "1D" in factor_returns:
        ...
        plotting.plot_cumulative_returns(
            factor_returns["1D"], period="1D", title=title, ax=ax
        )
    ...
    save_plot(gf.fig, "al_returns", render, **kwargs)
...
@plotting.customize
def create_information_tear_sheet(factor_data, group_neutral=False, by_group=False, render=None, **kwargs):
    """
    ...
    save_plot(gf.fig, "al_information", render, **kwargs)
...
@plotting.customize
def create_turnover_tear_sheet(factor_data, turnover_periods=None, render=None, **kwargs):
    """
    ...
    save_plot(gf.fig, "al_turnover", render, **kwargs)
...
def create_full_tear_sheet(
    ...
    create_returns_tear_sheet(
        factor_data, long_short, group_neutral, by_group, render=render, **kwargs
    )
    create_information_tear_sheet(
        factor_data, group_neutral, by_group, render=render, **kwargs
    )
    create_turnover_tear_sheet(factor_data, render=render, **kwargs)
