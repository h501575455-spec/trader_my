...
class RiskModel(BaseModel):
    ...
    def __init__(self, nan_option: str = "ignore", assume_centered: bool = False, scale_return: bool = False):
        """
        Args:
        ...
        if not isinstance(X, (pd.Series, pd.DataFrame)):
            ...
            if isinstance(X.index, pd.MultiIndex):
                if isinstance(X, pd.DataFrame):
                    X = X.iloc[:, 0].unstack(level="ts_code")  # always use the first column
                else:
                    X = X.unstack(level="ts_code")
            else:
                # X is 2D DataFrame
                pass
